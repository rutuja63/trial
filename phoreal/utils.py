...
def getNameParts(h5FileName):
    ...
    fileInfo = fileStruct(atlVersion, year, month, day, hour,
                          minute, second, trackNum, unknown, releaseNum, incrementNum)
...
def cantorPairing(arrayIn):
    ...
    vectorOut = 0.5 * (arrayIn[:, 0] + arrayIn[:, 1]) * \
        (arrayIn[:, 0] + arrayIn[:, 1] + 1) + arrayIn[:, 1]
...
def getIntersection2d(a_vec, b_vec, assume_unique=False):
    ...
    commonIDs, a_inds, b_inds = np.intersect1d(
        a_vec_IDs, b_vec_IDs, assume_unique, return_indices=True)
...
def getAtl08Mapping(atl03_ph_index_beg, atl03_segment_id, atl08_classed_pc_indx, atl08_classed_pc_flag, atl08_segment_id):
    ...
    atl03SegsIn08TF, atl03SegsIn08Inds = ismember(
        atl08_segment_id, atl03_segment_id)
    ...
    if not (module == 'mgrs' or module == 'pygeodesy'):  # None condition
        ...
        if DEG_ANTARCTIC <= lat < DEG_ARCTIC:
            ...
            if not (hemi == 'N' or hemi == 'S'):
                ...
                print('warning ({}): hemi={} is something other than N or S'.format(
                    module, hemi))
...
def getGeoidHeight(geoidData, atlTruthData):
    ...
    f = interpolate.interp2d(
        geoidData.lons, geoidData.lats, geoidData.geoidalHeights, kind='linear')
    geoidalHeights = interpolate.dfitpack.bispeu(
        f.tck[0], f.tck[1], f.tck[2], f.tck[3], f.tck[4], lonsIn, latsIn)[0]
...
def getRaster(x, y, z, resolution, method, fillValue=-999, time=[], xAllArray=[], yAllArray=[],
    ...
    colIndsNew = (
        (np.array(groupedData['xRnd']) - df_xRnd_min)/xResolution).astype(int)
    rowIndsNew = (
        (np.array(groupedData['yRnd']) - df_yRnd_min)/yResolution).astype(int)
...
def get_index(t, t0, tol=2.0, err_msg=True):
    ...
    if abs(t[index] - t0) > tol:
        if err_msg:
            print('warning: index not found: looking for %4.2f vs. found %4.2f' % (
                t0, t[index]))
        # return -1
    return index
...
def get_outlier_data(data):
    ...
    """
    Q1, Q2, Q3 = np.nanpercentile(data, 25), np.nanmedian(
        data), np.nanpercentile(data, 75)
    IQR = Q3-Q1
    return Q1, Q2, Q3, IQR
...
def get_root_dir(dir_type=None, username=None, debug=0):
    ...
    if type(dir_type) != type(None):
        ...
        if dir_type in dir_types:
            ...
            raise KeyError(
                dir_type + ' directory type unknown. Try one of', keys)
...
def get_sc_orient(file, delta_time=None):
    ...
    if found:
        ...
        raise KeyError('could not find [%s or %s]' %
...
def interp_vals(input_x, input_y, interp_x, removeThresh=False):
    ...
    f1 = interpolate.interp1d(
        input_x, input_y, kind='linear', fill_value='extrapolate')
...
def interp_vals2d(input_x, input_y, input_z, interp_x, interp_y, removeThresh=False, removeDuplicates=False):
    ...
    # Interpolate delta_time
    f1 = interpolate.interp2d(
        input_x, input_y, input_z, kind='linear', fill_value='extrapolate')
