...
from phoreal.utils import getAtl08Mapping, wgs84_to_utm_find_and_transform,\
    ...
    identify_hemi_zone, identify_hemi_zone, transform
            ...
            if idx == 0:
                ...
                if len(data.shape) == 2:
                    ...
                    for idx2 in range(0, cols):
                        df = pd.concat([df, pd.DataFrame(data[:, idx2], columns=[key.split('/')[-1] +
                                                         '_' + str(idx2)])],
                                       axis=1)
                else:
                    df = pd.concat([df, pd.DataFrame(data, columns=[key.split('/')[-1]])],
                                   axis=1)
    return df
            ...
            if key_info[idx][1] != 'Group':
                if idx == 0:
                    ...
                    if len(data.shape) == 2:
                        ...
                        for idx2 in range(0, cols):
                            df = pd.concat(
                                [df, pd.DataFrame(data[:, idx2], columns=[key.split('/')[-1] + '_' +
                                                  str(idx2)])], axis=1)
                    else:
                        df = pd.concat(
                            [df, pd.DataFrame(data, columns=[key.split('/')[-1]])], axis=1)
